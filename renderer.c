...
void draw_bkg(char i){
	for(int x=0;x<(r*c);x++){
		f[x]=i;}}
void draw_pix(char i,int x,int y){
	f[(((r-(y +1))*c)+x)]=i;}
void draw_lin(char i,float x1,float y1,float x2,float y2){
	...
	if(sqrt(((y2-y1)*(y2-y1)))<=sqrt(((x2-x1)*(x2-x1)))){
		...
		for(int x=y1;x<y2;x++){
			...
			draw_pix(i,y,x);}}}
void draw_rct(char i,int x1,int y1,int x2,int y2){
	for(int x=y1;x<y2;x++){
		draw_lin(i,x1,x,x2,x);}}
void draw_tri(char i,float x1,float y1,float x2,float y2,float x3,float y3){
	...
	for(int x=1;x<=points;x++){
		draw_lin(i,x1,y1,x2+interval_x*x,y2+interval_y*x);}}
void draw(){
	draw_bkg(' ');
